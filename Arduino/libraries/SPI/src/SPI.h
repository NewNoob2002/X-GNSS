...
#define SPI_I2S_WAIT_RX_FULL(spi_x)                         \
    do                                                      \
    {                                                       \
        while (!SPI_I2S_GET_FLAG(spi_x, SPI_I2S_RDBF_FLAG)) \
            ;                                               \
    } while (0)
#define SPI_I2S_WAIT_TX_EMPTY(spi_x)                        \
    do                                                      \
    {                                                       \
        while (!SPI_I2S_GET_FLAG(spi_x, SPI_I2S_TDBE_FLAG)) \
            ;                                               \
    } while (0)
#define SPI_I2S_WAIT_BUSY(spi_x)                         \
    do                                                   \
    {                                                    \
        while (SPI_I2S_GET_FLAG(spi_x, SPI_I2S_BF_FLAG)) \
            ;                                            \
    } while (0)
...
inline uint8_t spiFrequencyToClockDiv(uint32_t freq)
{
    uint32_t apb_freq = getApb1Frequency();
    const uint16_t divs[] = {2, 4, 8, 16, 32, 64, 128, 256, 512, 1024};
    uint8_t bestIdx = sizeof(divs)/sizeof(divs[0]) - 1; // 默认最大分频
    for (uint8_t i = 0; i < sizeof(divs)/sizeof(divs[0]); ++i) {
        if ((apb_freq / divs[i]) <= freq) {
            bestIdx = i;
            break;
        }
    }
